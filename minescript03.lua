...
print("please choose")
print("tunnel / stripmine-down")
local order = io.read()

local stepcounter = 0
local size = 50

if order == "tunnel" then
    local size = userInput("How wide does the tunnel need to be",1)
    local length = userInput("How long does the tunnel need to be (even number)",2)
    local high = userInput("how high?, minimum of 3",1)
    if high < 3 then
        error("you need at least 3 blocks high")
    end
    length = length/2
    print("the length will be ",length*2)
    ...
    for iter =1,length do
        rechtsaf()
        tunnelSection(size,high)
        linksaf()
        tunnelSection(size,high)
        restack()
        cleanup()
        local empty = countEmpty()
        if empty < 3 then
            chestDump("down") 
        end
    end
    ...
    if turtle.inspectDown() == false then
        ...
        print("something is blocking me below me.")
    end
    --move("forward",1)
    --turtle.turnRight()
    --turtle.turnRight()
...
elseif order == "stripmine-down" then
